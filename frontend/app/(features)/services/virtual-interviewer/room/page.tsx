...
type InterviewState = "connecting" | "ready" | "listening" | "speaking" | "recording" | "ended";
...
export default function InterviewRoomPage() {
    ...
    const connectWebSocket = async () => {
      try {
        ...
        try {
          ...
          console.warn("Failed to get user ID for websocket connection", err);
        }

        const extra = userId ? `?user_id=${encodeURIComponent(userId)}` : "";
          ...
          if (inputMode === "voice") {
            ...
            if (first === "{" || first === "[") {
              try {
                ...
                parsed = null;
                console.info("Received non-JSON string that starts with JSON char:", raw.slice(0, 120));
              }
            }
            ...
            if (parsed) {
              ...
              if (response.type === "interview_saved") {
                ...
                setTimeout(() => {
                  if (id) {
                    router.push(`/services/virtual-interviewer/database/${id}`);
                  } else {
              ...
              if (response.type === "transcription") {
                ...
                setMessages(prev => {
                  ...
                  for (let i = newMessages.length - 1; i >= 0; i--) {
                    if (newMessages[i].role === "user" && newMessages[i].content === "[Voice message]") {
                      newMessages[i].content = response.text;
                      break;
                    }
                  }
                  return newMessages;
              ...
              if (response.type === "response") {
                ...
                setMessages(prev => [...prev, { role: "interviewer", content: response.text }]);
                setState("speaking");
                
                // Play audio if available, otherwise speak the text
                if (response.audio) {
                  playAudioFromHex(response.audio, response.text);
                } else {
              ...
              if (text.includes("INTERVIEW COMPLETE")) {
                console.log("Agent: interview completed (server sent INTERVIEW COMPLETE)");
                ...
                console.info("Received formatted report text on room socket (suppressed)");
                setState("ready");
              } else {
                setMessages(prev => [...prev, { role: "interviewer", content: text }]);
                setState("ready");
  ...
  const getPersonaVoiceSettings = (personaKey: string) => {
    ...
    if (englishVoices.length > 1) {
      ...
      switch (personaKey) {
        ...
        case "sarah_williams": // Female
          assignedVoice = femaleVoices[1] || femaleVoices[0] || englishVoices[1] || englishVoices[0];
          break;
        case "marcus_johnson": // Male
          assignedVoice = maleVoices[0] || englishVoices[0];
          break;
        case "priya_patel": // Female
          assignedVoice = femaleVoices[2] || femaleVoices[1] || femaleVoices[0] || englishVoices[2] || englishVoices[0];
          break;
        case "jordan_lee": // Male
          assignedVoice = maleVoices[1] || maleVoices[0] || englishVoices[1] || englishVoices[0];
          break;
      }
      console.log(`Assigned voice for ${personaKey}:`, assignedVoice?.name, assignedVoice?.lang);
    ...
    const fallback = fallbackSettings[personaKey as keyof typeof fallbackSettings] || { pitch: 1.0, rate: 1.0 };
  ...
  const playAudioFromHex = async (hexString: string, fallbackText?: string) => {
    try {
      ...
      try {
        ...
        console.warn("Audio playback failed or was blocked; falling back to browser TTS", err);
  ...
  const toggleInputMode = () => {
    if (state === "recording" || state === "listening" || state === "speaking") {
            ...
            <div className="relative">
              <div
                className={`
                  ...
                  bg-gradient-to-br ${state === "recording" ? "from-red-500 via-red-400 to-red-500" : "from-primary via-accent to-primary"}
                  backdrop-blur-lg
                `}
                style={{
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center space-y-1">
                    <p className="text-sm font-medium text-foreground">
                      {isRecording ? "🎙️ Recording..." : state === "listening" ? "⏳ Processing..." : "Tap to speak"}
                    </p>
